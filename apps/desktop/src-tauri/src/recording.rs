...
pub async fn start_dual_recording(
  ...
  let media_recording_preparation = prepare_media_recording(&options, &audio_chunks_dir, &video_chunks_dir, &muxed_chunks_dir, &screenshot_dir, audio_name, state_guard.max_screen_width, state_guard.max_screen_height);
  ...
  println!("is_local_mode: {:?}", is_local_mode);

  if !is_local_mode {
      let muxed_upload = start_upload_loop(muxed_chunks_dir.clone(), options.clone(), "video".to_string(), shutdown_flag.clone(), state_guard.video_uploading_finished.clone());

      drop(state_guard);

      println!("Starting upload loop...");

      match tokio::try_join!(muxed_upload) {
          Ok(_) => {
              println!("Upload loop completed successfully.");
          },
          Err(e) => {
      ...
      println!("Skipping upload loop due to NEXT_PUBLIC_LOCAL_MODE being set to 'true'.");
  }
...
async fn prepare_media_recording(
  ...
  let muxed_file_path = muxed_chunks_dir.to_str().unwrap();
  let screenshot_dir_path = screenshot_dir.to_str().unwrap();
  media_recorder.start_media_recording(options.clone(), audio_file_path, video_file_path, muxed_file_path, screenshot_dir_path, audio_name.as_ref().map(String::as_str), max_screen_width, max_screen_height).await?;
  Ok(media_recorder)
}